...
@Slf4j
@Service
public class MediumParcelCalculatorServiceImpl implements VolumeParcelCalulatorService {
    ...
    public ParcelCostDto calculate(BigDecimal volume, VoucherDto voucherDto) {
        log.info("Parcel is medium.");
        BigDecimal cost;

        cost = volume.multiply(new BigDecimal(ruleBaseCostProperties.getMediumParcel()));
        ...
        return ParcelCostDto.builder().cost(cost.setScale(2, BigDecimal.ROUND_CEILING))
                ...
                .comment("Volume is less than " + ruleVolumeProperties.getMedium() + " cm3").build();
    }
}
